...
class OrderService {
  ...
  /// Get user's orders from Supabase or cache with pagination support
  Future<List<Order>> getUserOrders({int limit = 20, int offset = 0, bool skipCache = false}) async {
    try {
      ...
      // Try cache first for faster loading (only for first page and if skipCache is false)
  ...
  /// Remove orphaned orders from cache that no longer exist in database
  Future<void> _cleanupCache(List<Order> databaseOrders) async {
    try {
      final cachedOrders = await _cacheService.getCachedOrders();
      if (cachedOrders == null || cachedOrders.isEmpty) return;
      
      final databaseIds = databaseOrders.map((o) => o.id).toSet();
      final orphanedOrders = cachedOrders.where((cached) => !databaseIds.contains(cached.id)).toList();
  ...
  /// Delete an order and all its order items from the database
